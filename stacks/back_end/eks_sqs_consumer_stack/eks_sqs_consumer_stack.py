...
class EksSqsConsumerStack(cdk.Stack):
    def __init__(
        ...
        **kwargs
        ...
        trust_policy_stm_01 = _iam.PolicyStatement()
        trust_policy_stm_01.add_aws_account_principal(
            account_id=f"{cdk.Aws.ACCOUNT_ID}")
        trust_policy_stm_01.effect.ALLOW
        trust_policy_stm_01.add_actions("sts:AssumeRole")
        ...
        # Add Trust relation to ALLOW ANY user in the account to assume this role
        # There is NO way to append a trust policy after it is created. Only recreate?
        # The condition keys for STS are not very useful
        # https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_iam-condition-keys.html#condition-keys-sts
        self._events_processor_svc_accnt_role.assume_role_policy.add_statements(
            _iam.PolicyStatement(
                sid="AllowKedaSvcAccountToAssumeThisRole",
                actions=[
                    "sts:AssumeRole"
                ],
                principals=[
                    _iam.AccountPrincipal(f"{cdk.Aws.ACCOUNT_ID}")
                ]
            )
        )
